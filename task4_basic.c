...
int main(int argc, char *argv[]) {
    ...
    for (int iteration = 0; iteration < NUM_ITERATIONS; iteration++) {
        ...
        // Exchange prime numbers with neighbors in 6 directions
        MPI_Sendrecv(&my_prime, 1, MPI_INT, nbr_x_lo, 0, &right_prime, 1, MPI_INT, nbr_x_hi, 0, comm3D, MPI_STATUS_IGNORE);
        MPI_Sendrecv(&my_prime, 1, MPI_INT, nbr_x_hi, 1, &left_prime, 1, MPI_INT, nbr_x_lo, 1, comm3D, MPI_STATUS_IGNORE);
        MPI_Sendrecv(&my_prime, 1, MPI_INT, nbr_y_lo, 2, &bottom_prime, 1, MPI_INT, nbr_y_hi, 2, comm3D, MPI_STATUS_IGNORE);
        MPI_Sendrecv(&my_prime, 1, MPI_INT, nbr_y_hi, 3, &top_prime, 1, MPI_INT, nbr_y_lo, 3, comm3D, MPI_STATUS_IGNORE);
        MPI_Sendrecv(&my_prime, 1, MPI_INT, nbr_z_lo, 4, &rear_prime, 1, MPI_INT, nbr_z_hi, 4, comm3D, MPI_STATUS_IGNORE);
        MPI_Sendrecv(&my_prime, 1, MPI_INT, nbr_z_hi, 5, &front_prime, 1, MPI_INT, nbr_z_lo, 5, comm3D, MPI_STATUS_IGNORE);
